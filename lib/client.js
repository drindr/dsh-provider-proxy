...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    // Uses the raw settings remotes (like the built-in General settings page)
    ...
    // dsh 0.1.2: the connection handle no longer carries `.api`; the typert
    // remote namespace (ctx.remote.settings) is the raw-RPC path now.
    ...
    /**
     * Minimal settings-namespace scope talking straight to the host settings
     * remotes (settings.describe / settings.mutate over ctx.remote). Mirrors
     * the shape consumed by the section — status/value/base/user/revision/
     * writable plus subscribe, getSnapshot, load and set — but unlike
     * ctx.settingsScope.bind it is NOT gated on the page being loopback, so
     * the page works from the HTTPS front as well as from
     * http://127.0.0.1:3080. A failed read keeps the current status (callers
     * retry via load()).
     *
     * Wire shapes (dsh-api-settings-controller 0.1.2): describe() takes no
     * argument and resolves to {ok, value: {writable, hasDocument,
     * namespaces}}; mutate(ns, ops, expectedRevision?) is POSITIONAL and
     * resolves to {ok, value: <namespace view>}.
     */
    function createSettingsScope(remote) {
      ...
      async function read(generation) {
        ...
        if (!response.ok || disposed || generation !== readGeneration) return;
        const { namespaces, writable } = response.value;
      ...
      async function write(ops) {
        ...
        let response;
        try {
          response = await remote.settings.mutate(NS, ops, revision);
        } catch {
          await load();
          return;
        }
    ...
    function apply(ctx) {
      ...
      const scope = createSettingsScope(remote);
